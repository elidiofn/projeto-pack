...
import com.ufcg.si1.prefeitura.Prefeitura;
...
public class RestApiController {
	...
	public static final int SITUACAO_PREFEIRUTRA_CAOS = 2;

	private Prefeitura prefeitura = new Prefeitura(SITUACAO_PREFEITURA_NORMAL);
    ...
    public ResponseEntity<?> getSituacaoGeralQueixas() {
        ...
        if (prefeitura.getSituacao() == SITUACAO_PREFEITURA_NORMAL) {
        ...
        if (prefeitura.getSituacao() == SITUACAO_PREFEITURA_EXTRA) {
            if ((double) queixaService.numeroQueixasAbertas() / queixaService.findAllQueixas().size() > 0.1) {
                return new ResponseEntity<ObjWrapper<Integer>>(new ObjWrapper<Integer>(0), HttpStatus.OK);
            } else {
        ...
        if (prefeitura.getSituacao() == SITUACAO_PREFEIRUTRA_CAOS) {
            if ((double) queixaService.numeroQueixasAbertas() / queixaService.findAllQueixas().size() > 0.05) {
                return new ResponseEntity<ObjWrapper<Integer>>(new ObjWrapper<Integer>(0), HttpStatus.OK);
            } else {
                if ((double) queixaService.numeroQueixasAbertas() / queixaService.findAllQueixas().size() > 0.02) {
                    return new ResponseEntity<ObjWrapper<Integer>>(new ObjWrapper<Integer>(1), HttpStatus.OK);
                }
            }
        }
